...
class BarcodeReleaseHit {
  ...
  /// Carátula opcional (útil cuando el hit no viene de MusicBrainz/CoverArtArchive).
  ///
  /// - Si viene desde iTunes, por ejemplo, podemos mostrar una miniatura.
...
enum MbErrorKind { none, rateLimited, serviceUnavailable, network, unknown }

class MbTextSearchResult {
  final List<BarcodeReleaseHit> hits;
  final MbErrorKind errorKind;
  final int? statusCode;

  const MbTextSearchResult({
    required this.hits,
    this.errorKind = MbErrorKind.none,
    this.statusCode,
  });

  bool get ok => errorKind == MbErrorKind.none;
}
...
class BarcodeLookupService {
  ...
  static List<BarcodeReleaseHit> _parseReleaseSearch({required List releases, required String barcodeOrQuery}) {
  ...
  static Future<List<BarcodeReleaseHit>> searchReleasesByBarcode(String barcode) async {
    ...
    return _parseReleaseSearch(releases: releases, barcodeOrQuery: code);
  }
  ...
  static Future<List<BarcodeReleaseHit>> searchReleasesByText(String query) async {
    ...
    final data = jsonDecode(res.body) as Map<String, dynamic>;
    final releases = (data['releases'] as List?) ?? [];
    return _parseReleaseSearch(releases: releases, barcodeOrQuery: q);
  }

  /// Igual que [searchReleasesByText], pero devuelve info de error (rate limit/servicio caído, etc.)
  /// para poder mostrar mensajes mejores y hacer fallback.
  static Future<MbTextSearchResult> searchReleasesByTextDetailed(String query) async {
    final q = query.trim();
    if (q.isEmpty) return const MbTextSearchResult(hits: []);
    ...
    try {
      ...
      return const MbTextSearchResult(hits: [], errorKind: MbErrorKind.network);
    }

    if (res.statusCode != 200) {
      if (res.statusCode == 429) {
        return MbTextSearchResult(hits: const [], errorKind: MbErrorKind.rateLimited, statusCode: res.statusCode);
      }
      if (res.statusCode == 502 || res.statusCode == 503 || res.statusCode == 500) {
        return MbTextSearchResult(hits: const [], errorKind: MbErrorKind.serviceUnavailable, statusCode: res.statusCode);
      }
      return MbTextSearchResult(hits: const [], errorKind: MbErrorKind.unknown, statusCode: res.statusCode);
    }

    try {
      final data = jsonDecode(res.body) as Map<String, dynamic>;
      final releases = (data['releases'] as List?) ?? [];
      final hits = _parseReleaseSearch(releases: releases, barcodeOrQuery: q);
      return MbTextSearchResult(hits: hits);
    } catch (_) {
      return const MbTextSearchResult(hits: [], errorKind: MbErrorKind.unknown);
    }
  }
}
